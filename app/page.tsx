...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/15 bg-transparent p-4">
      ...
      <div className="mt-1 text-xs text-white/60">Placeholder</div>
    </div>
  );
}
...
export default function DashboardPage() {
  return (
    <div className="space-y-6">
      <Panel
        title="Dashboard"
        subtitle="Season 2026 • Current Round: — • Snapshot: LATEST • Updated: —"
      >
      ...
      <Panel title="Lock Insights" subtitle="Post-lock overview for the current round">
        <div className="rounded-xl border border-white/15 bg-transparent px-4">
        ...
        <div className="mt-3 text-xs text-white/60">
          We’ll compute this from the locked team selections (and show counts + %).
        </div>
      </Panel>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              >
                ...
                <div className="mt-1 text-xs text-white/60">Remaining: —</div>
              </div>
            ))}
          </div>
          <div className="mt-3 text-xs text-white/60">
            We’ll also highlight “most impactful chip so far” using chip_points_delta.
          </div>
        </Panel>
      </div>
    </div>
